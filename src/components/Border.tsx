...
type BorderVerticalProps<T extends React.ElementType> = {
  as?: T
  className?: string
  position?: 'left' | 'top'
  invert?: boolean
...
export function BorderVertical<T extends React.ElementType = 'div'>({
  as,
  className,
  position = 'left',
  invert = false,
  ...props
}: Omit<React.ComponentPropsWithoutRef<T>, keyof BorderVerticalProps<T>> &
  BorderVerticalProps<T>) {
  let Component = as ?? 'div'
