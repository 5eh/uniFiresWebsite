...
function NavigationRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-px bg-neutral-950 sm:bg-neutral-950">
      <Container>
...
function NavigationItem({
  ...
  return (
    <Link
      href={href}
      className="group relative isolate -mx-6 cursor-crosshair bg-neutral-950 px-6 py-10 even:mt-px sm:mx-0 sm:px-0 sm:py-16  sm:even:mt-0 sm:even:border-l sm:even:border-neutral-800 sm:even:pl-16"
    >
      {children}
      <span className="absolute inset-y-0 -z-10 w-screen cursor-crosshair bg-gradient-to-r from-blue-300/20 to-blue-900/20 opacity-0 transition group-odd:right-0 group-even:left-0 group-hover:opacity-100" />
    </Link>
  )
}
...
function Navigation() {
  return (
    <nav className="ld:text-5xl mt-px font-display text-2xl font-medium tracking-tight text-white sm:text-3xl md:text-4xl xl:text-6xl">
      ...
      <NavigationRow>
        <NavigationItem href="/proposals">PROPOSALS</NavigationItem>
        <NavigationItem href="/documents">DOCUMENTS</NavigationItem>
      </NavigationRow>
    </nav>
  )
}
